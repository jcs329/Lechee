...
public class IBeaconIntentProcessor extends IntentService {
				...
				if (!IBeaconManager.isInstantiated()) {
					...
					if (rangeNotifierClassName != null) {
						try {
						     ...
						     if (DEBUG) Log.d(TAG, "Automatically set range notifier: "+rangeNotifier);
						}
						catch (Exception e) {
					...
					if (monitorNotifierClassName != null) {
						try {
						     ...
						     if (DEBUG) Log.d(TAG, "Automatically set monitor notifier: "+monitorNotifier);
	...
	protected void onHandleIntent(Intent intent) {
		initialize();
		if (DEBUG) Log.d(TAG, "got an intent to process");
		...
		if (rangingData != null) {
			if (DEBUG) Log.d(TAG, "got ranging data");
		...
		if (monitoringData != null) {
			if (DEBUG) Log.d(TAG, "got monitoring data");
