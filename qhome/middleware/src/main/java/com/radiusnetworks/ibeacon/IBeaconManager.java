...
public class IBeaconManager {
	...
	public static IBeaconManager getInstanceForApplication(Context context) {
		if (!isInstantiated()) {
			if (DEBUG) Log.d(TAG, "IBeaconManager instance craetion");
	...
	private String rangingCallbackAction() {
		...
		if (DEBUG) Log.d(TAG, "ranging callback action: " + action);
		return action;
	}

	private String monitoringCallbackAction() {
		String action = context.getPackageName() + ".DID_MONITORING";
		if (DEBUG) Log.d(TAG, "monitoring callback action: " + action);
		return action;
	}
	...
	static class IncomingHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			default:
				...
				if (DEBUG) Log.d(TAG, "Got a ranging callback with data: " + data);
				if (DEBUG) Log.d(TAG, "Got a ranging callback with " + data.getIBeacons().size() + " iBeacons");
				if (data.getIBeacons() != null) {
					...
					if (manager.rangeNotifier != null) {
						if (DEBUG) Log.d(TAG, "Calling ranging notifier on :" + manager.rangeNotifier);
