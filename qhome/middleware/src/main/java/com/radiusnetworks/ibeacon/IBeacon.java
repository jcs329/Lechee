...
public class IBeacon { 
	 ...
	 * Example: device: 00:02:72:C5:EC:33
	 */
	...
	@Override
	public int hashCode() { 
		return Integer.valueOf(deviceMACAddress.copyValueOf(deviceMACAddress.toCharArray(), 15, 2), 16);
	}
	...
	public boolean equals(Object that) {
		...
		return (thatIBeacon.getMajor() == this.getMajor() && thatIBeacon.getMinor() == this.getMinor() && thatIBeacon.getProximityUuid().equals(this.getProximityUuid()) && thatIBeacon.getDeviceMACAddress().equals(this.getDeviceMACAddress()));
	}
	/**
	...
	public static IBeacon fromScanData(byte[] scanData, int rssi, BluetoothDevice device) {
		...
		if (((int)scanData[5] & 0xff) == 0x4c &&
			...
			if (DEBUG) Log.d(TAG, "This is not an iBeacon advertisment.  The bytes I see are: "+bytesToHex(scanData));
			return null;
		}
		...
		*/
		iBeacon.proximityUuid = hexString;
		iBeacon.deviceMACAddress = device.getAddress();
	...
	protected IBeacon(IBeacon otherIBeacon) {
		...
		this.deviceMACAddress = otherIBeacon.deviceMACAddress;
	}
	...
	protected static double calculateAccuracy(int txPower, double rssi) {
		...
		if (DEBUG) Log.d(TAG, "calculating accuracy based on rssi of "+rssi);
		...
		if (ratio < 1.0) {
			...
			if (DEBUG) Log.d(TAG, " avg rssi: "+rssi+" accuracy: "+accuracy);
			return accuracy;
		}
	}	
