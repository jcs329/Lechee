...
static StunAtrString _password; // LiangBin add, 20150204
static void ice_send_binding_request(IceCheckList *cl, IceCandidatePair *pair, const RtpSession *rtp_session)
{
	...
	//<<< LiangBin add, get ice-pwd
    snprintf(_password.value, sizeof(_password.value) - 1, "%s", ice_check_list_local_pwd(cl));
    _password.sizeValue = strlen(_password.value);
    //>>> LiangBin add, 20150204
...
static void ice_send_binding_response(const RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *dest)
{
	...
	len = stunEncodeMessage(&response, buf, len, &password); // LiangBin add, set ice-pwd, 20150204, cancel in 20150217
	if (len > 0) {
...
static int ice_check_received_binding_request_attributes(const RtpSession *rtp_session, const OrtpEventData *evt_data, const StunMessage *msg, const StunAddress4 *remote_addr)
{
	...
	if (!msg->hasUsername) {
		...
		return -1;
		// LiangBin add, ignore to check USERNAME, 20150204, cancel in 20150217
	}
