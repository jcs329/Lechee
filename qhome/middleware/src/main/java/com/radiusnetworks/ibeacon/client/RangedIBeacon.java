...
public class RangedIBeacon extends IBeacon{
	...
	private double calculateRunningAverage() {
		...
		if (DEBUG) Log.d(TAG, "Running average rssi based on "+size+" measurements: "+runningAverage);
	...
	protected void addRangeMeasurement(Integer rssi) {
		this.rssi = rssi;
		addMeasurement(rssi);
		if (DEBUG) Log.d(TAG, "calculating new range measurement with new rssi measurement:"+rssi);
