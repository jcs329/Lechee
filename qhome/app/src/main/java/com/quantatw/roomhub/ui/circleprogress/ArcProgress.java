package com.quantatw.roomhub.ui.circleprogress;
...
public class ArcProgress extends View {
    ...
    private Drawable finish_check_image;
    private float checked_margin_left;
    private float checked_margin_top;
...
//        default_suffix_padding = Utils.dp2px(getResources(), 4);
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        finish_check_image = attributes.getDrawable(R.styleable.ArcProgress_arc_finish_check_image);
        checked_margin_left = attributes.getDimension(R.styleable.ArcProgress_arc_checked_margin_left, 0);
        checked_margin_top = attributes.getDimension(R.styleable.ArcProgress_arc_checked_margin_top,0);
    }
    ...
    public float getPercent() {
        ...
        float val = ((float)this.progress/(float)this.max)*100;
//        Log.d("Circle","this.progress="+this.progress+",this.max="+this.max+",getPercent="+val);
        return val;
    }
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        float startAngle = 180 - arcAngle / 2f;
...
//        String text = String.valueOf(getProgress());
        String text = String.valueOf((int)getPercent());
...
//            canvas.drawText(suffixText, getWidth() / 2.0f  + textPaint.measureText(text) + suffixTextPadding, textBaseline + textHeight - suffixHeight, textPaint);
            if(getProgress()>=getMax() && finish_check_image != null) {
                Bitmap.Config config = finish_check_image.getOpacity() != PixelFormat.OPAQUE ? Bitmap.Config.ARGB_8888
                        : Bitmap.Config.RGB_565;
                Bitmap bitmap = Bitmap.createBitmap(finish_check_image.getIntrinsicWidth(),finish_check_image.getIntrinsicHeight(),
                        config);
                Canvas drawableCanvas = new Canvas(bitmap);
                finish_check_image.setBounds(0, 0, finish_check_image.getIntrinsicWidth(), finish_check_image.getIntrinsicHeight());
                finish_check_image.draw(drawableCanvas);

                canvas.drawBitmap(bitmap,((getWidth() - textPaint.measureText(text)) / 2.0f+textPaint.measureText(text)+checked_margin_left)
                        ,rectF.centerY()+textHeight+checked_margin_top, textPaint);
            }
        }
